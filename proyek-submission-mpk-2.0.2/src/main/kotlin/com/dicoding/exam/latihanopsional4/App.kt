...
fun getMiddleCharacters(string: String): String {
    ...
    if (strValue.length > 0 && strValue.length <= 100) {
        val listOfString = strValue.toList()
        if (listOfString.size % 2 == 0) {
            ...
            result = middleWord.joinToString(separator = "")
        } else {
            val middleList = listOfString.size / 2
            val middleWord = listOfString.get(middleList)
            result = middleWord.toString()
        }
    } else {
        println("Inputan Anda melebihi kapasitas")
    }
    return result
}
